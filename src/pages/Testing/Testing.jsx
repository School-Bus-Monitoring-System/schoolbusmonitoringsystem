  ...
  useEffect(() => {
    ...
    const handleNewMessage = (topic, message) => {
      ...
      if (topic === mpuTopic) {
        ...
        const accelerometerX = parseFloat(mpuValues[0]).toFixed(2);
        const accelerometerY = parseFloat(mpuValues[1]).toFixed(2);
        const accelerometerZ = parseFloat(mpuValues[2]).toFixed(2);

        // Gyroscope values
        const gyroscopeX = parseFloat(mpuValues[3]).toFixed(2);
        const gyroscopeY = parseFloat(mpuValues[4]).toFixed(2);
        const gyroscopeZ = parseFloat(mpuValues[5]).toFixed(2);
    ...
    <Layout>
      <div className="flex flex-col m-8 rounded-3xl shadow p-5">
        ...
        <div className="flex w-full h-1/4">
          <img
            className="rounded-3xl shadow-lg md:w-1/2 transform -scale-y-100 -scale-x-100"
            src={imageUrl}
            alt="....."
          />
        </div>
